...
def main():
    ...
    file_weights = os.path.join(
        "output", "model_weights", subfolder, weight_folder, weight_name
    )
    ...
    if fromVideo:
        if target_dataset == "LEDOV" or target_dataset == "UAV123":
            list_video = pd.read_csv(os.path.join("data", target_dataset, "test.csv"))[
                "0"
            ].values.tolist()
            list_video.sort()
        else:
            list_video = [
                v
                for v in os.listdir(path_video)
                if os.path.isfile(os.path.join(path_video, v))
            ]
    else:
        list_video = [
            v
            for v in os.listdir(path_frames)
    ...
    for v in list_video:
        ...
        if not os.path.isdir(destination_path):
            ...
            if len(list_frames) >= 2 * len_temporal - 1:
                ...
                snippet = []
                for i in tqdm(
                    range(len(list_frames)),
                    desc=f"number of frames: {len(list_frames)}",
                ):
                    img = list_frames[i]
                    ...
                    if i >= len_temporal - 1:
                        ...
                        if i < original_length:  # only for the original frames
                            ...
                            overlap[i].paste(
                                Image.fromarray(img),
                                mask=Image.fromarray(frames_mask[i]),
                            )

                        if i < 2 * len_temporal - 2:
                            j = i - len_temporal + 1
                            frames_mask[j] = process(
                                model, torch.flip(clip, [1]), j, destination_path
                            )
                            img = cv2.applyColorMap(frames_mask[j], cv2.COLORMAP_HOT)
                            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                            overlap[j].paste(
                                Image.fromarray(img),
                                mask=Image.fromarray(frames_mask[j]),
                            )
                        del snippet[0]
                ...
                for idx in range(len(overlap)):
                    overlap[idx].save(
                        os.path.join(
                            destination_path, "images", "%04d.jpg" % (idx + 1)
                        ),
                        format="JPEG",
                        quality=100,
                    )
...
def resized_frames_from_video(v, path_video):
    ...
    while success:
        image = cv2.resize(
            image, dsize=(image_size[1], image_size[0]), interpolation=cv2.INTER_CUBIC
        )
...
def transform(snippet):
    ...
    snippet = snippet.view(1, -1, 3, snippet.size(1), snippet.size(2)).permute(
        0, 2, 1, 3, 4
    )
    return snippet
...
def process(model, clip, idx, path_output):
    ...
    smap = (smap.numpy() * 255.0).astype(int) / 255.0
    smap = gaussian_filter(smap, sigma=7)
    smap = (smap / np.max(smap) * 255.0).astype(np.uint8)
